...
var ctx = canvas.getContext("2d");
    ...
    playerC.x = clamp(playerC.x, worldBounds.minX, worldBounds.maxX);
    playerC.y = clamp(playerC.y, worldBounds.minY, worldBounds.maxY);

    // center the camera around the player,
    // but clamp the edges of the camera view to the world bounds.
 camX = clamp(playerC.x - canvas.width/2, worldBounds.minX, worldBounds.maxX - canvas.width);
 camY = clamp(playerC.y - canvas.height/2, worldBounds.minY, worldBounds.maxY - canvas.height);
...
if(adj===0){adj=0.0001;}
  ...
  st = st;
   ...
   this.bulletss.push(new bullets(this.x+(62.5/2)-((62.5/3.1622)/2),this.y+(62.5/2)-((62.5/3.1622)/2),"reg",this,"enemy",st,sh,this.rotation+90));}else{
    if(adj===0){adj=0.0001;}
...
enemy.prototype.point = function(){
        <?php if(&_COOKIE["Difficulty"]==="Insane")?>
...
<?php else if(&_COOKIE["Difficulty"]==="Hard")?>
<?php  endif?>
}
toggle=function(e){
...
if(!this.hasfound){
 ...
 var leng=this.pathdupes.length;
  ...
  enemy.prototype.move = function(){
    this.pathfind();
      if(raycast([this.x,this.x+62.5],[this.y,this.y+62.5],[playerC.x,playerC.x+62.5],[playerC.y,playerC.y+62.5])&&this.insight){
this.insight=true;
opp = Math.abs((this.y+(62.5/2))-(playerC.y+(62.5/2)));
adj = Math.abs((this.x+(62.5/2))-(playerC.x+(62.5/2)));
if(adj===0){adj=0.0001;}
...
for(var daps=0; daps<enemies.length;daps++){
    ...
    }else if(enemies[dadps].bulletss[0].exists){}}
}
